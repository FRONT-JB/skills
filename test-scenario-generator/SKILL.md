...
**시나리오 ID 참고:**
- 이 단계에서는 카테고리 접두사(예: `LGN`, `SGN`)만 지정
- 최종 순번(`-001`, `-002` 등)은 Step 3에서 기존 DB를 조회한 후 자동 확정
...
2. 기존 시나리오 조회 · 채번 · 중복 검사
   - notion-database-query로 기존 시나리오 전체 조회
   - 시나리오 ID 자동 채번:
     a. 카테고리별로 기존 시나리오 ID에서 최대 순번 파악
        (예: LGN-001, LGN-002가 있으면 최대 순번 = 2)
     b. 새 시나리오에 다음 순번 자동 부여
        (예: 다음 로그인 시나리오는 LGN-003부터 시작)
     c. 해당 카테고리에 기존 시나리오가 없으면 001부터 시작
   - 중복 검사:
     a. 새 시나리오의 시나리오명을 기존 시나리오명과 비교
     b. 동일하거나 유사한 시나리오가 존재하면 사용자에게 알림:
        - 기존 시나리오 ID, 시나리오명을 표시
        - 등록 진행 / 건너뛰기 / 기존 시나리오 업데이트 중 선택 요청
     c. 사용자가 건너뛰기를 선택한 시나리오는 등록 목록에서 제외
...
```
기존 DB 시나리오: [LGN-001, LGN-002, LGN-003, FID-001]
새 시나리오 카테고리: 로그인 2건, 아이디 찾기 1건, 대시보드 1건

→ 자동 채번 결과:
  - 로그인: LGN-004, LGN-005 (기존 최대 003 → 004부터)
  - 아이디 찾기: FID-002 (기존 최대 001 → 002부터)
  - 대시보드: DSB-001 (기존 없음 → 001부터)
```

**중복 검사 예시:**

```
새 시나리오명: "유효한 이메일/비밀번호로 로그인 성공"
기존 DB에 유사 시나리오 발견:
  - LGN-001: "유효한 이메일/비밀번호로 로그인 성공" (동일)

→ 기존 시나리오와 동일하거나 유사한 항목이 있습니다:
  | 기존 ID  | 기존 시나리오명                        | 유사도 |
  |----------|---------------------------------------|--------|
  | LGN-001  | 유효한 이메일/비밀번호로 로그인 성공     | 동일   |
