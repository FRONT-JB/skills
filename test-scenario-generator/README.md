...
> 시나리오 ID의 순번은 기존 DB 데이터를 조회하여 자동으로 부여됩니다. 동일 카테고리의 마지막 순번 다음 번호부터 시작합니다.

## 중복 시나리오 검사

시나리오 등록 전 기존 DB에 동일하거나 유사한 시나리오가 있는지 자동으로 확인합니다:

1. 새 시나리오명과 기존 시나리오명을 비교
2. 유사 시나리오 발견 시 기존 ID와 시나리오명을 표시
3. 사용자에게 처리 방법 선택 요청:
   - **그대로 등록** - 새 ID를 부여하여 별도 등록
   - **건너뛰기** - 해당 시나리오를 등록하지 않음
   - **기존 시나리오 업데이트** - 기존 항목을 덮어쓰기
