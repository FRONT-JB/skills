...
pack: see `meta.json` `ui` · `packVersion` **1.3.10**
...
| 허용 (영문 유지) | 반드시 한글 |
|------------------|------------|
| role 키, task/dispatch id, 파일 경로, CLI 로그 **인용**, **`scv_line` 인용** | 단계 전환, 대기 이유, 완료/실패 요약, FINAL 본문, **task display-name, wait shell description, 터미널 타이틀**, 내레이션 `다음:` |

### 엔진 타입 → 사용자 문구 (필수 · pack 1.3.10)
...
```markdown
**【요구사항 접수 】** "I read you." — 오늘 현황 탭만 재구성 · 다음: 계획 작성
```
...
```markdown
**【구현 】** "Orders received." — 배치 1 코딩 중 · 다음: 품질 게이트
```
...
| 금지 | soft-wait/생존신호 마다 반복 · 진행 내레이션 **영어 only** · 엔진 타입 bare 노출 · `【scv · …】` · 긴 두 줄 본문 기본 |
...
### Human gate + UX (중복 방지 · pack 1.3.6)

```markdown
**【계획 승인 대기 】** "Orders, Cap'n?" — plan-review r3 APPROVED · 승인 대기
```
...
```markdown
**【대기 】** "Affirmative." — gate 보완·plan 수정 중 · 다음: 작업 완료 대기
```
...
**세션 라벨 (1.3.7):** Audit 는 항상 **새 탭** + 위 라벨. 계획 검토/코드 리뷰 탭 이름을 감사에 재사용 금지.
