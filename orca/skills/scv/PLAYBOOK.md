...
- packVersion: `meta.json` 의 `packVersion` (**1.3.3**). 동작 계약 SSOT는 이 파일; 변경 이력은 `LESSONS.md` / `meta.notes.changelog_*`
...
| 허용 (영문 유지) | 반드시 한글 |
|------------------|------------|
| `worker_done`, role, task id, 파일 경로, CLI 로그 인용, **`scv_line` 인용** | 단계 전환, 대기 이유, 완료/실패 요약, decision_gate, FINAL 본문 |

### 진행 내레이션 포맷 (필수 · pack 1.3.3)

**두 줄** 고정. 접두 `scv ·` **넣지 않음**.

```text
【 <한글 phase 라벨> 】 "<scv_line>"
<한글 한 줄 설명 — 누가/무엇/대기 이유>
```

**여백:** `【` 뒤·`】` 앞 **공백 1칸** 고정 (앞뒤 동일). 예: `【 대기 】` · `【 계획 작성 】` · `【 구현 】`.
...
```text
【 계획 검토 】 "I read you."
plan 산출물 확인 완료. Codex plan-review를 dispatch 합니다.
```

```text
【 구현 】 "Orders received."
Codex 배치 구현 중 — worker_done까지 대기합니다.
```

| 규칙 | 내용 |
|------|------|
| 발화 시점 | **phase 진입 · role dispatch · user gate · blocked · hang recovery · FINAL** 만 |
| 여백 | `【 ` + 라벨 + ` 】` — 앞뒤 공백 1칸 동일. `【대기】` / `【 대기】` / `【대기 】` 금지 |
| 금지 | soft-wait timeout / heartbeat 마다 반복 · 진행 내레이션 **영어 only** · `【scv · …】` 접두 |
| 저장 | `scv_line` 은 **표시 전용** — `run.json` / `.scv/state` 에 영속 저장 금지 |
| 워커 | Claude/Codex task spec · `worker_done` body 에 대사 **주입 금지** (팀장 채팅만) |

### phase → 한글 라벨 · `scv_line` (고정 테이블)

원작 StarCraft SCV 음성 대사(레거시 FRONT-JB/scv README 표)를 Orca phase에 매핑. **문구 임의 창작 금지** — 아래 표만 사용.
...
대기 중(soft timeout 체크포인트, 사용자 재고지 1회 필요 시) 예시:

```text
【 대기 】 "Affirmative."
계획 작성 워커 작업 중 (heartbeat≠완료). worker_done 대기.
```

금지 예: `Heartbeat received…`, `Plan worker_done received…` 같은 **진행 내레이션 전부 영어**.

### 터미널 타이틀 (한글 · 필수 · pack 1.3.3)

팀장이 스폰하는 worker pane 제목은 **짧은 한글 고정 라벨**. UTF-8 허용. lifecycle 키로 쓰지 말 것.

| role (meta, 영문) | `terminalTitle` | 라운드 변형 |
|-------------------|-----------------|-------------|
| init | `초기화` | — |
| plan | `계획 작성` | — |
| plan-review | `계획 검토` | `계획 검토 · 2` |
| implement | `구현` | `구현 · 배치2` (배치 있을 때만) |
...
orca terminal create --worktree active \
  --title "계획 작성" \
  --command '<meta.workers[role].command>' --json

# 2+ 워커 (split 에는 --title 없음 → 즉시 rename)
...
| 규칙 | 내용 |
|------|------|
| create | `--title` = 위 표 (문장형 금지 · idempotent `(title, role)` 재사용) |
| split | 직후 **`rename` 필수** |
| 핑퐁 | 같은 handle 재사용 · 라운드만 `· 2` 등으로 rename |
| 금지 | 제목 퍼지로 task 재사용 · 긴 SCV 영문 문장을 탭 타이틀로 사용 |
| display-name | `task-create --display-name` 도 동일 한글 라벨 **권장** (`--task-title` 은 `[scv:$RUN_ID] …` 영문 유지) |
...
| 순서 | 동작 |
|------|------|
| 1 | **첫 워커만** `terminal create` + **한글 `--title`** (위 표) |
| 2+ | **`terminal split`** + **`rename` 한글 타이틀** (위 표) |
| 예외 | split 실패 복구 1회 · 사용자 “탭으로” · 다른 worktree |

- direction: 기본 `vertical` · 리뷰 쌍 `horizontal`
- **타이틀:** role → 한글 고정 라벨 (`계획 작성`, `구현`, …). create `--title` / split 직후 `rename`. 영문 role slug 를 탭에 쓰지 않음.
...
| 사용자 안내 | phase 진입 시에만 두 줄 연출: `【 대기 】 "Affirmative."` + 한글 설명. soft-timeout 마다 반복 금지. |
