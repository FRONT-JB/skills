---
name: zealot-orchestration
description: >-
  Use the dedicated zealot Orca orchestration snapshot for structured
  multi-agent coordination: threaded
...
# zealot Orca Inter-Agent Orchestration
...
Parse JSON RPC envelopes by command-specific nested paths: task = `result.task.id`, dispatch = `result.dispatch.id`, terminal create = `result.terminal.handle`, and terminal split = `result.split.handle`. Never use the root RPC `id` as a lifecycle identifier.
...
- While supervising workers manually, use **exactly one** rolling `check --wait --types worker_done,escalation,decision_gate --timeout-ms <n>` owner loop. Never start a second parallel `check --wait` (or a sleep-poll that also calls `check --wait`) — concurrent waiters steal messages from each other and waste coordinator turns.
- Prefer rolling windows of **60s–180s**, not a single 15-minute shell block. Host agent shells often background long commands; a short rolling wait returns control for liveness checks.
...
- Heartbeats and visible terminal activity mean the worker is alive, not done. Do **not** put `heartbeat` in the same `--types` filter as completion unless you intentionally drain them. Prefer `--types worker_done,escalation,decision_gate` and treat heartbeat via occasional `--peek` or terminal preview. Do not stop, close, kill, or restart a worker just because it has not produced a completion message yet.
...
- Group addresses include `@all`, `@idle`, `@claude`, `@codex`, `@opencode`, `@gemini`, `@droid`, `@cursor`, and `@worktree:<id>`.
...
## Coordinator Efficiency (anti-waste)

Lessons from multi-worker research/implement runs. Apply on every supervised loop.

### Do less as coordinator

- **Do not re-do the worker's job.** Light inventory (`gh api users/X`, repo list) to *shape tasks* is fine. Full README/clone deep-dives belong to research workers — coordinator re-research doubles token time.
- **Decompose for independence**, not role theater. If design is “markdown IA for a report”, prefer one research-capable agent over spawning Claude solely for ceremony. Three agents that thrash on hang recovery are slower than two healthy ones.
- **Spawn only ready work.** Create implement tasks with `--deps` so they stay `pending` until inputs exist; do not spin Codex early just to “warm” it.

### One wait owner

```bash
# Good: single rolling loop (N completions expected)
for i in $(seq 1 40); do
  orca orchestration check --wait --types worker_done,escalation,decision_gate \
    --timeout-ms 90000 --json > /tmp/orca-msg.json
  # parse /tmp/orca-msg.json LINE-WISE (see NDJSON below) → decision_gate / completions
  # on timeout: liveness snapshot (task-list + one terminal show), then continue
done
```

- **Never** stack two coordinator shells both running `check --wait`.
- **Never** `echo "$JSON" | python3 <<'PY'` — heredoc steals stdin; write JSON to a file, then `python3 -c` / `python3 <<'PY'` reading that file.
- Prefer `task-list --brief --json` for sweeps; full `task-list` only when editing specs.
- Before starting a new wait loop, kill **duplicate waiter shells only** (never kill worker terminals/tasks).

### Parse `check --wait` NDJSON (mandatory)

`check --wait --json` may emit **multiple lines**: `_keepalive` / `_heartbeat` NDJSON records, then a final result object. Loading the **entire stdout** with one `json.loads` often fails with `Extra data`.

| Rule | Detail |
|------|--------|
| Line-wise | Split stdout on newlines; `json.loads` each non-empty line |
| Skip | Objects with `_keepalive` or `_heartbeat`; blank lines; per-line parse errors |
| Accept | Last valid object (or any object with `result.messages`) as the wait result |
| Never | Treat parser failure as worker failure — soft-recheck `task-list` + `terminal show` |

### Lifecycle stragglers (late mail)

- Authority is payload **`taskId` + `dispatchId`**, not terminal title.
- Drop / silent-dedupe: `worker_done` or `heartbeat` for **already-completed** tasks, or `dispatchId` that is not the current active dispatch for that task.
- Do **not** re-open completed tasks or re-wait on late replay. After run close, do not spam the user with residual completed lifecycle mail.
- Unresolved **human** `decision_gate` is never dropped.

### Post-inject liveness (mandatory)

`tui-idle` + `injected: true` is **not** proof the agent is working.

After every `dispatch --inject`:

1. Wait **45–90s** wall time (one short `check --wait` window is fine for wall clock only — do not treat keepalive as completion).
2. `orca terminal show|read` the assignee. Healthy: tool calls, file paths, heartbeat, or non-trivial preview growth. Unhealthy: only spinner (`⏺`), only launch line (`❯ claude …` / Starting MCP), bare shell prompt, **“Update ran successfully / restart Codex”**, **Ready with no tools** for ≥90s after inject.
3. If unhealthy → **Hung worker recovery** (below). Do not wait 15–60 minutes on a frozen first turn.

### Hung worker recovery

When a dispatch is stuck with no productive activity:

1. `task-update --id <task> --status ready` (not `failed` unless abandoning). Avoid cascading `pending`→`failed` while experimenting.
2. Create a **fresh** terminal with a known-good agent command (prefer an agent that already completed a task this session).
3. `dispatch --inject` to the new handle. **Do not re-inject into the frozen pane** — including when `dispatch-show` still says `active` / Ready (active-dispatch stuck is a distinct hang signal).
4. Optional: leave the frozen pane alone; do not spam `terminal send y/p` hoping to unstick first-turn hangs.
5. After **one** failed recovery on the same role, reassign role to a fallback agent (mode pack may name fallbacks) rather than infinite Claude/Codex restarts.
6. Coordinator may complete a narrow deliverable **only** as last-resort recovery when (a) raw inputs already exist, (b) worker cannot write (sandbox/UI), and (c) user goal is blocked. Record this under FINAL → Decisions as recovery, not as “worker_done”. If resuming a worker after coordinator partial edits, put **uncommitted SSOT paths** in the task spec and keep **one edit owner**.

### Codex / Claude approval UI

- Codex prompts like `Would you like to run… Yes, proceed (y)` often **do not** receive `orca terminal send --text y`. Keys land in the **chat compose queue** (`Messages to be submitted after next tool call`) instead of the approval widget.
- **Prevention (preferred):** launch Codex workers with non-interactive approvals when the mode pack allows, e.g. `-a never` and a sandbox that can write required paths (`-s workspace-write` inside the worktree, or `-s danger-full-access` only when the task must write outside the workspace and the pack explicitly opts in).
- **Prevention for Claude permission stalls:** `--dangerously-skip-permissions` or `--permission-mode bypassPermissions` when the pack opts in for trusted local workers.
- **If already stuck on approval:** prefer interrupt + re-dispatch with fixed launch flags over repeated `y`/`p` sends.
- **Codex self-update:** if the pane shows package update success / “restart Codex” / drops to a shell, treat as hung — **fresh terminal**, do not inject into that session.
...
Wait for `tui-idle` before dispatching, then treat idle as a **minimum** readiness bar, not full proof of health. Always pass `--timeout-ms` on waits. Real coding tasks can take 15-60 minutes, but a worker that never starts tools after inject is hung — recover early (see Post-inject liveness). During supervision, use a **single** rolling `check --wait` owner. If a window returns no matching message, inspect `task-list`, `terminal read`/`show`, or expected artifact paths as a liveness checkpoint; if the terminal is still working or producing activity, keep waiting instead of retrying the task.
...
Coordinator: confirm `orca status --json`, inspect `task-list`/`dispatch-show` if inheriting state, then choose either a manual loop (`task-create` → worker → `dispatch --inject` → **post-inject liveness** → **one** rolling `check --wait`) or `orchestration run`. Kill duplicate background wait loops before starting a new one.
