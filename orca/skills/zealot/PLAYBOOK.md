# zealot Orchestration Mode (Orca) — Global

Claude = **coordinator**. Workers = **Claude/init + Claude/plan + Codex/plan-review + Codex/implement + Claude/code-review + Codex/review-fix + Claude/release**.
...
- 표시/연출 SSOT: `$HOME/.orca/zealot/UX.md` · `meta.json` `ui`
- 홈: `$HOME/.orca/zealot/`
- 템플릿: `$HOME/.orca/zealot/templates/`
- 프로젝트 오버레이: `.orca/zealot.md` (있으면) · `AGENTS.md` (있으면)
- 런타임 상태(레포 내부): `.zealot/state/$RUN_ID/` (**gitignore**, 커밋 금지)
- packVersion: `meta.json` 의 `packVersion` (**1.0.0**). 변경 이력: `LESSONS.md` / `meta.notes.changelog_*`

## 사용자 대면 언어 (필수 · 한글)

coordinator 진행·질문·FINAL = **한국어**. role/path/task id/CLI 영문 허용 · `zealot_line` 인용 영문.

**연출·표·예시 SSOT = `$HOME/.orca/zealot/UX.md`** (+ `meta.json` `ui`). 이 절은 요약만.

| 표면 | 규칙 (상세 → UX.md) |
|------|---------------------|
| 채팅 | 한 줄: `**【한글 phase 】** "zealot_line" — 요약 · 다음: …` · `】` 앞 공백 1칸만 (`【대기 】`) |
| 탭 / `--display-name` | 한글 역할 라벨 (`계획 작성` …) |
| `--task-title` | `[zealot:$RUN_ID] 한글 phase · slug` |
| wait shell description | `계획 작성 완료 대기` · bare `worker_done` / `Rolling wait…` 금지 (UX 1.0.0) |
| 발화 시점 | phase 진입·dispatch·gate·blocked·FINAL 만 (soft-wait 스팸 금지) |
| **사람 결정 게이트** | **AskUser 정확히 1회** (아래 절) · 본문 선택지 재질문 금지 |

## Human decision gates · AskUser (필수)

Claude 팀장 세션의 **`ask_user_question` (AskUser)** 가 사람 확인 UI다.
...
| 워커(Claude/Codex) AskUser? | **아니오** — 사람 게이트는 **coordinator(Claude) only** |
...
구조화 UI와 `fallback-text-once`는 `fixtures/gates-option-ids.json`의 동일한 `optionId`와 `resultCode`를 사용한다. 미해결 gate에서는 어떤 worker도 dispatch하지 않는다. fallback 본문 선택지는 해당 gate에서 한 번만 허용한다.

### AskUser 금지 · 예외 (free-text / 비게이트)

| 상황 | UI |
|------|-----|
| bare `/zealot` · empty seed | **free-text 1회** (추정 옵션 메뉴 선제 금지 — 기존 intake) |
...
2) 선택: UX 라벨 한 줄 (질문 문장 없이)
   예: **【계획 승인 대기 】** "What battle calls?" — plan-review APPROVED · 승인 대기
...
```text
명시적 사용자 지시  >  프로젝트 `.orca/zealot.md` docsLanguage  >  전역 기본(ko)
```
...
| `.orca/zealot.md` 설명 문장 | resolved (기본 ko) |
| `.zealot/state/**` | 한글 권장 (커밋 안 함) |
...
오버라이드: `.orca/zealot.md`에 `docsLanguage: en` 명시 시 영문 문서 허용 (위반 아님). 질문/FINAL은 여전히 한글.

## 언제 쓰나

트리거: operative standalone **`/zealot` only**. bare `zealot`, `zealot-harness`, substring, 인용, 코드 펜스, URL 속 `/zealot`은 기동하지 않는다.
...
3. `.zealot/` 가 `.gitignore`에 있는지 확인 · **`.zealot/**` 절대 스테이징 금지**
4. `.orca/zealot.md` 에 기본 gate·docsLanguage·versionPolicy·`audit` 확인
5. (권장) claude alias / codex 터미널 플래그 스모크

**RUN_ID / state dir는 non-empty seed 확보 후에만** 발급 (bare `/zealot` 만으로 orphan state 금지). seed 확보 후:

6. `RUN_ID=$(date +%Y%m%d-%H%M%S)-<slug>` · `[zealot:$RUN_ID]` 접두 · `.zealot/state/$RUN_ID/` 생성  
...
|------|-------|-----------|---------|-------|------|------|
| coordinator | claude | — | (세션) | 인터뷰·브리프·dispatch·gate 판정·FINAL | FINAL · decisions | plan/코드 **설계 본문** 미작성 |
| init | claude | edit | `claude --model opus --dangerously-skip-permissions` | docs 골격·ARCHITECTURE 초안 | docs/** scaffolding | 소스 수정 금지 · 사용자 승인 후만 · 템플릿 참조 |
...
| plan-review | codex | review-only | `codex -m gpt-5.6-sol -c model_reasoning_effort="high" -a never -s danger-full-access` | plan 검증 | `.zealot/.../plan-review/codex.md` | 파일 수정 금지 |
...
| code-review | claude | review-only | `claude --model opus --dangerously-skip-permissions` | 코드 리뷰 | `.zealot/.../code-review/claude-round-N.md` | 수정 금지 |
...
| release | claude | edit | `claude --model opus --dangerously-skip-permissions` | 7a docs · 7b push 확인 | changelog·CR·ARCHI·push-status | 코드 설계 본문 금지 · push는 확인 후 |
...
- coordinator handle은 `terminals[]`, dispatch target, reclaim allowlist에 넣지 않는다.
- init, plan, code-review, release는 모두 Claude여도 서로 다른 role/phase handle을 생성한다.
- coordinator가 직접 할 수 있는 일은 intake, brief, dispatch, gate 기록, inventory/improvements 합본, FINAL뿐이다. plan/code/release 산출 대행은 `decisions.md` 예외 키 없이는 금지한다.
- same-role + same-phase loop만 resume하며 Audit의 Claude와 Codex는 항상 fresh handle이다.
- 기계 검증용 계약은 `fixtures/d1-run.json`부터 `fixtures/d5-session-policy.md`까지를 따른다.
...
**Empty Goal / bare `/zealot` 는 오류가 아니다.** 다만 **추정 옵션 메뉴를 먼저 띄우지 않는다.**

#### 0a. seed prompt 확보 (prompt-first)

1. 사용자 메시지의 operative standalone `/zealot` 명령과 빈 `Goal:` 접두를 제외한 본문을 **seed prompt** 로 추출. 인용·펜스·URL occurrence는 트리거가 아니다.
2. **seed가 비어 있지 않으면** 그 텍스트를 인터뷰 입력으로 사용.  
   - 한 줄 Goal · 긴 배경/제약 모두 동일.  
   - 팀장이 가정·범위 초안을 **한 단락 요약** 후, 모호한 점만 gstack 인터뷰 (선택지는 보조).
3. **seed가 비어 있으면** (`/zealot` only 등):  
...
6. gstack 인터뷰로 Goal·범위·brief 확정 → `.zealot/state/$RUN_ID/brief/plan-brief.md`.
...
orca orchestration task-create \
  --task-title "[zealot:$RUN_ID] 계획 작성 · <짧은-slug>" \
  --display-name "계획 작성" \
...
**`--display-name` = 한글 역할 라벨 필수** · `--task-title` = `[zealot:$RUN_ID]` + 한글 phase + slug.  
**이번 런 task-create id만 dispatch.** 제목 퍼지 매칭 금지.

#### LIFECYCLE 블록 (필수 · inject preamble 보강)
...
| 전환 | handoff 최소 (`.zealot/state/$RUN_ID/handoff/` 또는 기존 산출 경로) |
|------|------|
...
팀장(zealot) 탭에 `Orchestration Messages` 가 **쌓여 멈추는 것**은 정상이 아니다. 아래를 강제한다.
...
| peer 런 | runtime-global task 목록에 다른 `[zealot:$OTHER]` 가 보이면 soft warn 한 줄. **title 만으로 공유 worktree 단정 금지**(task row에 branch 없음; terminal registry join 후에만 위험 승격). 값 변할 때만 고지 |
...
Goal → gstack 인터뷰 → brief: `.zealot/state/$RUN_ID/brief/plan-brief.md`  
인터뷰 중 plan 본문·코드 작성 금지.

#### 2. plan 작성

- claude-plan · `docs/plans/YYYY-MM-DD_<slug>.plan.md`
- 템플릿: `$HOME/.orca/zealot/templates/plan.ko.md` (resolved ko)
...
- **기본:** claude-release 워커 dispatch (task/dispatch/worker_done 증거)
- coordinator 대행 시 decisions에 `release:coordinator-exception` + 사유·파일 목록
- 버전 · changelog · CR 승격 · ARCHITECTURE — 경로/bump는 plan 또는 `.orca/zealot.md` (`versionPolicy`: 기본 `root-only`)
...
**산출** (`.zealot/state/$RUN_ID/audit/`, 기본 비커밋)
...
4) evidence escrow → .zealot/state/$RUN_ID/reclaim/midrun-log.md append
...
| plan-review | `.zealot/.../plan-review/*.md` + verdict + worker_done 소비 |
| implement | commits + quality-gate md + gate PASS 증거 + worker_done |
| code-review | `.zealot/.../code-review/claude-round-N.md` + verdict |
...
orca orchestration send \
  ...
  --report-path ".zealot/state/<RUN_ID>/…/report.md" \
  --json
```
...
```json
{
  ...
  "head": "<sha>",
  "reportPath": ".zealot/state/<RUN_ID>/quality-gate/….md",
  "filesModified": ["…"]
}
```
...
- bare `/zealot` 만으로 orphan RUN_ID/state 생성
- 사용자 대면 진행 영어 only · `【zealot · …】` 접두 · soft-wait/heartbeat 마다 대사 스팸
- human decision gate를 본문 선택지로만 묻고 AskUser 생략 · 같은 게이트 질문 2회
- bare `/zealot` empty seed에 추정 AskUser 메뉴 선제 (free-text 1회 규칙 위반)
- 터미널 타이틀 영문 slug only · split 후 rename 누락 · 타이틀로 task 퍼지 매칭
- `--display-name` 영문 only (`zealot-plan` 등) · `--task-title` 에 한글 phase 누락
